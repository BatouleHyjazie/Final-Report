...
To explore frequentist and bayesian statistics, as an exercise, I will be running the same model using these two different approaches. Because the response that is measured is the proportion of surviving Osmia offspring, the data will is not normally distributed but binomially distributed. This was verified using both a histogram (***Figure_***) of the distribution of the frequency of *Osmia* spp. survival ratios and a q-q plot (***Figure_***). Thus, all models will be run using a binomial distribution.

```{r 2.2 Statistical Methods Normality, fig.cap="\\label{fig:figs}Q-Q Plot of *Osmia* spp. survival ratio}

qqPlot(Osmia$surv.ratio, xlab="Normal Quantiles", ylab="Osmia spp. Survival Ratio")
```

```{r 2.2 Statistical Methods Normality, fig.cap="\\label{fig:figs}Histogram of *Osmia* spp. survival ratio}
...
```{r 2.2.2 Bayesian Approach Initial Chain, echo = TRUE}
...
```{r 2.2.2 Bayesian Approach Final Chain, echo = TRUE}
...
In **Figure 2**, a simple comparison of the means of *Osmia* spp. survival across habitat types shows that there is not much apparent difference in survival rates among different natural versus orchard populations. However, this will be confirmed in further analyses of the statistical results. 

```{r 3.1 Osmia Survival Plot, fig.cap="\\label{fig:figs}*Osmia* spp. survival (mean ± sd) in natural sites versus orchard sites using un-transformed and un-standardized data"}
...
In both the frequentist and bayesian approaches, we reach similar statistical results: 
...
```{r 3.2.1.1 Key Assumptions Chi-Square, echo = TRUE}
...
```{r 3.2.1.1 Key Assumptions Wald's Chi-Square Test GLMM vs GLM, echo = TRUE}

Osmiaglmmvm <- anova(osmiaglmm, osmiaglm)
Osmiaglmmvm
```

To test the effect of nesting the random effects of site and block ID on model fit, the Wald's Chi-Square test demonstrates that the nested random value causes AIC values to increase from 198.03 to 765.46. All AIC comparisons are summarized in ***Table 1***.

```{r 3.2.1.1 Key Assumptions Model Comparison, fig.cap="\\label{fig:figs}Comparison Between Tested Model Types"}

export_summs(osmiaglmm, osmiaglmmtest, osmiaglm, error_format = "(CI [{conf.low}, {conf.high}]", ci_level = 0.95,
             model.names = c("Final GLMM", "Un-Nested GLMM", "GLM"), coefs = c("Intercept" = "(Intercept)", "Orchard Site Type" = "s.typeo"))
```


```{r Key Assumptions Wald's Chi-Square Test GLMM vs GLMM Nested, , echo = TRUE}
...
```{r Key Assumptions GLMM Un-Nested Singularity Test, echo = TRUE}
...
```{r Osmia Overdispersion Tests, echo = TRUE}
...
```{r 4.3 Limitations of Analysis Power Analysis, echo = TRUE}
